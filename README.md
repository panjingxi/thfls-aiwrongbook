...
一个专为学生打造的 AI 智能错题本，帮你告别手抄、获得优质解析、快乐开启提分之旅！

## ✨ 主要功能

- **🤖 AI 秒解难题**：随手一拍，AI 自动识别题目，生成生成详细解析、知识点标签和同类变式练习。
- **📚 分门别类管理**：按科目（数学、物理、英语等）管理错题，找题查题更迅速。
- **📝 举一反三特训**：AI 会根据你的错题生成变式题。这道题会了？换个数试试，彻底消灭知识盲点！
- **🖨️ 告别手写抄题**：一键导出错题 PDF 或直接打印。支持自定义打印内容（答案/解析/知识点）和图片缩放，省下时间去刷两道新题！
- **📊 学习进度条**：可视化展示你的掌握情况，看着“未掌握”变成“已掌握”，成就感满满。
- **⚙️ 强悍 AI 适配**：内置超级好用的 **Google Gemini** 和 **OpenAI** 接口，小白也能轻松上手的极简配置。
- **🏷️ 智能标签系统**：自动提取知识点标签，支持自定义分类。
- **🔍 秒搜秒选**：支持按掌握状态、时间、知识点、年级、试卷等级等多种条件精准筛选。
- **🔐 多人私密空间**：支持多用户注册，数据安全隔离，保护你的学习小秘密。
- **🛡️ 纯净后台管理**：简单的管理员后台，搞定用户管理一切需求。
